...
import { AvaCrypto, Chain, TestEnv, Token, Wallet, BigMath } from "@liquidity_lab/effect-crypto";
...
testEffect("Should create and initialize pool", (t) => {
  ...
  return Effect.gen(function* () {
    ...
    const feeAmount = Adt.FeeAmount.MEDIUM;
    const expectedPrice = Either.getOrElse(Price.makeTokenPriceFromRatio(WETH, USDC, BigMath.Ratio(Big("4000"))), (err) =>
      t.fail(`Failed to create TokenPriceUnits: ${err}`),
    );
    ...
    const existingPoolPriceOpt = yield* Pool.createAndInitialize(
      Either.getOrElse(Price.makeTokenPriceFromRatio(WETH, USDC, BigMath.Ratio(Big("5000"))), (err) =>
        t.fail(`Failed to create TokenPriceUnits: ${err}`),
      ),
      feeAmount,
    );
